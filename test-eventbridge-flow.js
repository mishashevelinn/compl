// Test script for the EventBridge complaint workflow
import fetch from 'node-fetch';
import { setTimeout } from 'timers/promises';

// We'll get these URLs from the SST deployment output
// For now, using the previous URLs as placeholders
...
async function monitorComplaintStatus(id, targetStatus = 'RESOLVED', maxAttempts = 12, intervalSeconds = 5) {
...
// Run the EventBridge workflow test
async function runEventBridgeTest() {
  try {
    console.log('=== STARTING EVENTBRIDGE WORKFLOW TEST ===');
    ...
    // as the EventBridge -> SQS -> Lambda flow processes it
    ...
    if (createdComplaints.length > 0) {
      ...
      // This might take a while as the event goes through EventBridge -> SQS -> Lambda
    ...
    console.log('\n=== EVENTBRIDGE WORKFLOW TEST COMPLETED ===');
  } catch (error) {
    console.error('EventBridge workflow test failed:', error);
  }
}
